...
def callbacks(c):
    try:
        ...
        # -------- MAIN MENUS (ثابتة لا تتعدل) --------
        if data == "menu_tasbeeh":
            bot.send_message(chat_id, "📿 اختر ذكر:", reply_markup=tasbeeh_menu())
        elif data == "menu_fixed":
            bot.send_message(chat_id, "🌿 اختر نوع الأذكار:", reply_markup=fixed_menu())
        elif data == "back_main":
            bot.send_message(chat_id, "📿 القائمة الرئيسية", reply_markup=main_menu())
        elif data == "menu_stats":
            kb = InlineKeyboardMarkup()
            kb.add(InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="back_main"))
            bot.send_message(chat_id, format_stats(user), reply_markup=kb)
        elif data == "menu_global" and uid == ADMIN_ID:
            kb = InlineKeyboardMarkup()
            kb.add(InlineKeyboardButton("⬅️ القائمة الرئيسية", callback_data="back_main"))
            bot.send_message(chat_id, global_stats(), reply_markup=kb)

        # -------- TASBEEH (تحديث نفس الرسالة) --------
